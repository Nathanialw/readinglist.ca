...
func GetFavouriteBooks(name string) (threads []Book, err error) {
	//get all the books from a readinglist
	//use the uid to get the books from the books table
	bookuids, err := contentDB.Query("select uid from favouritedbooks where user = ?", name)
...
func ShowFavouritedBooks(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	generateHTML(w, data, "favouritebooks", "favouritebooks")
}
