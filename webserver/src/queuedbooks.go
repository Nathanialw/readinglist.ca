...
func GetQueuedBooks(name string) (threads []Book, err error) {
	//get all the books from a readinglist
	//use the uid to get the books from the books table
	bookuids, err := contentDB.Query("select uid from queuedbooks where user = ?", name)
...
func ShowQueuedBooks(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	generateHTML(w, data, "queuedbooks", "queuedbooks")
}
